<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=6D28D9&height=120&section=header" alt="Cabeçalho roxo em formato de onda" />

<p align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Poppins&weight=700&size=34&duration=3500&pause=1000&color=A855F7&center=true&vCenter=true&width=900&lines=Ol%C3%A1%2C+eu+sou+Marley+Thales;Desenvolvedor+Full-Stack+em+forma%C3%A7%C3%A3o;ADS+na+FATEC+Zona+Leste;Java+%7C+Python+%7C+Desenvolvimento+Web" alt="Apresentação animada de Marley Thales" />
  </a>
</p>

<h2 align="center">Bem-vindo(a) ao meu GitHub! 👋🏾</h2>

<p align="center">
  Sou técnico em <strong>Desenvolvimento de Sistemas</strong> pela ETEC de Itaquera e estudante de
  <strong>Análise e Desenvolvimento de Sistemas</strong> na FATEC Zona Leste. Gosto de transformar ideias
  em aplicações web funcionais, organizadas e pensadas para resolver problemas reais.
</p>

<p align="center">
  Atualmente, aprofundo meus conhecimentos em <strong>Java</strong> e <strong>Python</strong>, enquanto desenvolvo
  projetos full-stack com JavaScript, React, Node.js, Firebase e Supabase.
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/marley-thales-soares-guimar%C3%A3es-de-souza-00780b389">
    <img src="https://img.shields.io/badge/LinkedIn-6D28D9?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="https://portfolio1-pi-three.vercel.app">
    <img src="https://img.shields.io/badge/Portf%C3%B3lio-111827?style=for-the-badge&logo=vercel&logoColor=A855F7" alt="Portfólio" />
  </a>
  <a href="mailto:marleytfox@gmail.com">
    <img src="https://img.shields.io/badge/Email-6D28D9?style=for-the-badge&logo=gmail&logoColor=white" alt="E-mail" />
  </a>
</p>

---

### 🚀 Sobre mim

- 🎓 Cursando **Análise e Desenvolvimento de Sistemas** na FATEC Zona Leste.
- 💻 Técnico em **Desenvolvimento de Sistemas** pela ETEC de Itaquera.
- 🎯 Buscando uma oportunidade de **estágio em desenvolvimento de software**.
- ☕ Aprofundando meus estudos em **Java e Python**.
- 🌐 Experiência prática na criação de aplicações web full-stack.
- 📚 Interesse em arquitetura de software, microsserviços, CI/CD e computação em nuvem.
- 🌎 Inglês em desenvolvimento e boa compreensão de espanhol.

### 🛠️ Tecnologias e ferramentas

<div align="center">
  <img src="https://skillicons.dev/icons?i=java,python,js,html,css,react,nodejs,express,vite,firebase,supabase,postgres,git,github,vscode&perline=8" alt="Java, Python, JavaScript, HTML, CSS, React, Node.js, Express, Vite, Firebase, Supabase, PostgreSQL, Git, GitHub e VS Code" />
</div>

### 📌 Projetos em destaque

#### 🎟️ EventFlow

Plataforma web para criação e gerenciamento de eventos, com autenticação, controle de dados, geração de ingressos em PDF e QR Code.

`JavaScript` `Node.js` `Firebase` `Vercel`

#### 📚 PHD Escola Virtual

Plataforma educacional full-stack com autenticação, recuperação de senha, notificações e gerenciamento de dados acadêmicos.

`React` `Node.js` `Supabase`

#### ✂️ Nobre Corte — em desenvolvimento

Landing page e sistema de agendamento para uma barbearia, com organização de clientes, barbeiros, serviços, horários e prevenção de conflitos entre reservas.

`React` `Vite` `Node.js` `Supabase`

---

### 📊 Desenvolvimento no GitHub

<div align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=marleybob12&show_icons=true&include_all_commits=true&count_private=true&hide_border=true&bg_color=0D1117&title_color=A855F7&text_color=FFFFFF&icon_color=8B5CF6" alt="Estatísticas do GitHub de Marley Thales" />
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=marleybob12&layout=compact&langs_count=8&hide_border=true&bg_color=0D1117&title_color=A855F7&text_color=FFFFFF" alt="Linguagens mais usadas por Marley Thales no GitHub" />
</div>

<br />

<div align="center">
  <img width="95%" src="https://github-readme-activity-graph.vercel.app/graph?username=marleybob12&bg_color=0D1117&color=A855F7&line=8B5CF6&point=FFFFFF&area=true&hide_border=true" alt="Gráfico de contribuições de Marley Thales" />
</div>

<p align="center">
  <em>Aprendendo, construindo e evoluindo um projeto de cada vez.</em>
</p>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=6D28D9&height=120&section=footer" alt="Rodapé roxo em formato de onda" />
